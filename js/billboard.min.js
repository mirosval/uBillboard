...
"number")k=p[d].duration;clearTimeout(r.transitionComplete);r.transitionComplete=setTimeout(function(){var b="default";typeof e.link==="string"&&e.link!==""&&e.link!=="#"&&(b="pointer");j.stop().html(e.html).css({top:"0px",left:"0px",opacity:1,backgroundImage:"url("+e.bg+")",cursor:b});h.stop().hide();g.trigger("udsBillboardTransitionDidComplete",a)},k);n=a;g.trigger("udsBillboardSlideDidChange",n);b.pauseOnVideo&&i[n].transition=="none"||(o&&f.animateCountdown(i[n].delay),(b.autoplay||o)&&s.play())}}},
...
g.trigger("didChangePlayingState",{playing:o})},pause:function(){o=!1;g.trigger("didChangePlayingState",{playing:o});r.nextSlideAnimation!==null&&clearTimeout(r.nextSlideAnimation);typeof l!=="undefined"&&l!==null&&l.hide()},playpause:function(){o?s.pause():s.play()}},f={initSlides:function(){i=[];c(".uds-bb-slide",g).each(function(a,b){var d={delay:parseInt(c(".uds-delay",b).remove().text(),10),linkTarget:c(".uds-link-target",b).remove().text(),transition:c(".uds-transition",b).remove().text(),direction:c(".uds-direction",
...
""?{backgroundColor:"transparent",backgroundImage:"url("+a.bg+")"}:{backgroundColor:a.bgColor,backgroundImage:""}},handleEmbeddedContent:function(a){a.transition=="none"&&(j.html(a.html),a=c(">*",j),a.css({position:"absolute",top:parseInt(b.height,10)/2-a.attr("height")/2,left:parseInt(b.width,10)/2-a.attr("width")/2,margin:"auto"}))},pos:function(a){return Math.abs(parseInt(a,10))+"px"},neg:function(a){return"-"+Math.abs(parseInt(a,10))+"px"}},v={none:{delay:c.noop},left:{delay:function(){for(var a=
Math.ceil(parseInt(b.width,10)/parseInt(b.squareSize,10)),e=0;e<a;e++)c(".uds-column-"+e,g).delay(e*(600/a))}},right:{delay:function(){for(var a=Math.ceil(parseInt(b.width,10)/parseInt(b.squareSize,10)),e=0;e<a;e++)c(".uds-column-"+e,g).delay(600-e*(600/a))}},top:{delay:function(){for(var a=Math.ceil(parseInt(b.height,10)/parseInt(b.squareSize,10)),e=0;e<a;e++)c(".uds-row-"+e,g).delay(e*(600/a))}},bottom:{delay:function(){for(var a=Math.ceil(parseInt(b.height,10)/parseInt(b.squareSize,10)),e=0;e<
a;e++)c(".uds-row-"+e,g).delay(600-e*(600/a))}},center:{delay:function(){for(var a=Math.ceil(parseInt(b.height,10)/parseInt(b.squareSize,10)),e=Math.ceil(parseInt(b.width,10)/parseInt(b.squareSize,10)),d=0;d<e;d++)for(var k=0;k<a;k++){var f=Math.sqrt(Math.pow(d-e/2+0.5,2)+Math.pow(k-a/2+0.5,2))/Math.sqrt(Math.pow(e/2,2)+Math.pow(a/2,2));c(".uds-square-"+(k*e+d)).delay(700*f)}}},randomSquares:{delay:function(){m.each(function(){c(this).delay(Math.random()*700)})}},spiralIn:{delay:function(){for(var a=
...
10)/parseInt(b.squareSize,10)),d=0;d<e;d++)for(var f=0;f<a;f++){var g=(f*a+(f%2==0?d:e-d))/(a*a+e/2);c(".uds-square-"+(f*e+d)).delay(700*g)}}},zigzagVertical:{delay:function(){for(var a=Math.ceil(parseInt(b.height,10)/parseInt(b.squareSize,10)),e=Math.ceil(parseInt(b.width,10)/parseInt(b.squareSize,10)),d=0;d<e;d++)for(var f=0;f<a;f++){var g=(d*e+(d%2==0?f:a-f))/(e*e+a/2);c(".uds-square-"+(f*e+d)).delay(700*g)}}}},p={none:{duration:0,direction:"",setup:function(){h.hide()},perform:c.noop()},fade:{duration:1E3,
direction:"",setup:function(){m.css({opacity:0})},perform:function(){v[this.direction].delay();m.animate({opacity:1},{duration:500})}},crossFade:{duration:1E3,direction:"",setup:function(){m.css({opacity:0})},perform:function(){v[this.direction].delay();j.animate({opacity:0},{duration:1E3});m.animate({opacity:1},{duration:500})}},slide:{duration:700,direction:"right",setup:function(){c(".uds-bb-slides",g).css({overflow:"hidden"});j.css({top:"0px",left:"0px"});if(this.direction==="left")h.show().css({top:"0px",
left:f.neg(b.width)});else if(this.direction==="top")h.show().css({top:f.neg(b.height),left:"0px"});else if(this.direction==="bottom")h.show().css({top:f.pos(b.height),left:"0px"});else if(this.direction==="top")h.show().css({top:"0px",left:f.pos(b.width)});else if(this.direction==="zigzagHorizontal"){parseInt(b.squareSize,10);var a=parseInt(b.height,10);m.each(function(){c(this).css({opacity:0,top:parseInt(c(this).css("top"),10)-a+"px"})});v[this.direction].delay()}else if(this.direction==="zigzagVertical"){parseInt(b.squareSize,
10);var e=parseInt(b.width,10);m.each(function(){c(this).css({opacity:0,left:parseInt(c(this).css("left"),10)-e+"px"})});v[this.direction].delay()}else h.show().css({top:"0px",left:f.pos(b.width)})},perform:function(){var a={duration:700,easing:"easeInOutQuad"};if(this.direction==="left")j.animate({left:f.pos(b.width)},a),h.animate({left:"0px"},a);else if(this.direction==="top")j.animate({top:f.pos(b.height)},a),h.animate({top:"0px"},a);else if(this.direction==="bottom")j.animate({top:f.neg(b.height)},
a),h.animate({top:"0px"},a);else if(this.direction==="top")j.animate({left:f.neg(b.width)},a),h.animate({left:"0px"},a);else if(this.direction==="zigzagHorizontal"){parseInt(b.squareSize,10);var e=parseInt(b.height,10);m.each(function(){c(this).animate({opacity:1,top:parseInt(c(this).css("top"),10)+e+"px"},a)})}else if(this.direction==="zigzagVertical"){parseInt(b.squareSize,10);var d=parseInt(b.width,10);m.each(function(){c(this).animate({opacity:1,left:parseInt(c(this).css("left"),10)+d+"px"},a)})}else j.animate({left:f.neg(b.width)},
a),h.animate({left:"0px"},a)}},scale:{duration:1100,direction:"",setup:function(){var a,e;if(this.direction==="right")a=f.pos(parseInt(b.height,10)/2),e=f.pos(b.width);else if(this.direction==="left")a=f.pos(parseInt(b.height,10)/2),e="0px";else if(this.direction==="top")a="0px",e=f.pos(parseInt(b.width,10)/2);else if(this.direction==="bottom")a=f.pos(b.height),e=f.pos(parseInt(b.width,10)/2);else if(this.direction==="center")a=f.pos(parseInt(b.height,10)/2),e=f.pos(parseInt(b.width,10)/2);else{if(this.direction===
"zigzagHorizontal"){var d=parseInt(b.squareSize,10);m.each(function(){c(this).css({top:parseInt(c(this).css("top"),10)+d/2+"px",left:parseInt(c(this).css("left"),10)+d/2+"px",height:"0px"})})}else this.direction==="zigzagVertical"?(d=parseInt(b.squareSize,10),m.each(function(){c(this).css({top:parseInt(c(this).css("top"),10)+d/2+"px",left:parseInt(c(this).css("left"),10)+d/2+"px",width:"0px"})})):(d=parseInt(b.squareSize,10),m.each(function(){c(this).css({top:parseInt(c(this).css("top"),10)+d/2+"px",
